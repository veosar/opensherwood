...
the pick-up of one scroll; the walk to it crosses no zone with a handler. The outcome is tainted (the
scroll-pickup radius and the door stubs are hypotheses, ADR-0008), so the tests assert the taint as well
as the win; the run is recorded and played back as a `ReplayV1` (checkpoints equal, no divergence).
