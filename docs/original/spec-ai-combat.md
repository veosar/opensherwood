...
Status: `draft`, revision 7 (answers Codex review 42 of revision 6 at commit `0347564`, blob
`0cd519412b987b8e726a9f2c028a7ecf14da5ce2`, and closes the section 9.2 exclusions it named as far as the code
was read; revisions 2..6 answered reviews 16, 22, 27, 33 and 40; awaiting clearance). Build: GOG English edition, `Robin Hood.exe` SHA-256
...
`01a0b406-a088-76b0-b80e-f5d4a5abd73d` (spec-reviewer role only); reviews 16, 22, 27, 33, 40 and 42 are
...
`...2026-09-18-codex-review-40-...` and `...2026-09-18-codex-review-42-spec-ai-combat.md`; review 22 examined
revision 2 at commit `e966b05` (blob `e10208494c95caf92c2b356357329a61cf014c13`), review 27 revision 3 at
`e7b2cd4` (blob `c9040a2a406e8525fbd6aae32241b81fe0c72a6c`), review 33 revision 4 at `392e4a3` (blob
`58639d3662664b1654895783afed961d130b480b`), review 40 revision 5 at `5349b4f` (blob
`b78704dd5afebdd6122888e689d24eb9ca22a0b4`), review 42 revision 6 at `0347564`
(`0347564160cfbf585eb934a6074c1f6502ec39cb`, blob `0cd519412b987b8e726a9f2c028a7ecf14da5ce2`). Publication
approval: pending (separate from factual approval). Revision 7 adds two delegated readers (identity block).
...
- **Delegated readers**: six forked sub-sessions of session `a2931a3a5b130742c` (same role, same
  ...
  2026-09-13 as six background tasks of session `a2931a3a5b130742c`, each with the harness task identifier
  given below and a transcript file `tasks/<identifier>.output` in that session's scratchpad (private,
  auditable by the maintainer): (1) profile fields - task `ad0cda5be1e033c60`, `profile-fields.md`, exposure:
  the profile loaders, the
  difficulty scaler and the stat consumers; (2) perception - task `a2343bbdd0cf8a299`, `perception.md`,
  exposure: the cone, the scan, hearing, line of sight, identification; (3) the non-attacking state machine
  - task `a33f7f4eb716acd89`, `state-machine.md`, exposure: the event dispatchers, the default / wondering /
  seeking / sleeping / fleeing handlers, the civilian AI; (4) the attacking branch - task
  `a6ee73efa93438a99`, `attacking.md`, `substate_ids.txt`, exposure: decisions, morale, targets, the
  sword-fight step, the figure chooser, tactics; (5) rails, orders and natives - task `a15e5f847dd520d59`,
  `rails-orders-natives.md`, `state-ids.txt`, `natives-map.tsv`, exposure: the rail interpreter, the order
  pipeline, the natives and callbacks; (6) combat mechanics - task `a8404997eb858f6f1`, `combat.md`,
  exposure: engagement, resolution, stun, energy, the bow, projectiles, purses, wasps; and, launched on
  2026-09-18 for revision 7: (7) the unread state branches - task `ace012c2966e00904`, `gaps-state.md`,
  exposure: menacing, the tower-guard alert and the officer's rally, the colleague-found handler, the beggar
  rule, the civilian sight / hearing branches, the initialisers, the stun setter's callers, the level tick's
  element pass; (8) the order pipeline - task `a2e6c3e843c0ffd9b`, `gaps-orders.md`, exposure: the click
  dispatcher and its intents, the context actions, the right click, the act-on-arrival modifier, the stroke
  recogniser, the search and tie executors, the training-target object. No identity is unrecoverable. Their
...
- **Arithmetic contract** (AI-008, observed on the float expressions cited, disassembled where the export
  ...
  returned** (AI-066 steps 5 -> 6 -> result), and the entry's value is stored after (2g) x W x 0.01 (AI-068);
  in the aim test (AI-176) the horizontal displacement dx, the scaled displacement
  1.33 x dy, the height difference dz and the tangent are stored singles, **the reach itself is kept extended
  ...
  7 (the 15 / 159 contributions, the 54 / 108 / 60 delays, the same-height and downhill aim cases, the 18 / 48
  scoring case) were computed under (a)..(d) and match the instructions read; general bit-equivalence of
  every float path with the original is **not** demonstrated and is not claimed. OpenSherwood's chosen
  arithmetic (section 8 item 4: `f32` storage at the boundaries of (c), `f64` intermediates) reproduces those
  fixtures; behaviour that depends on rounding boundaries not listed is withheld from clearance until a
  fixture settles it (9.2 item 18).

### 2.2 The random stream

One global stream (AI-005): state x 214013 + 2531011 (mod 2^3^2), result = (state >> 16) & 0x7fff. It is seeded
...
| post 3..4, 5..6, 7..8, 9..10 | u16 x4 | `s0..s3` | stimulus susceptibilities in **file order purse, apple, beer, whistle** (percent): the purse word is the guard of the money reaction and enters the brawl-continuation formula; the apple word / 2 is the chase count; the beer word guards the ale reaction; the whistle word guards the whistle reaction (> 1 for cover) and sets the knight's search radius `(whistle - 2) x 400 / 98` px. See 9.1 for the review's contrary order and the evidence | 00438cb0, 004468a0, 0044cb40, 00424d50 |
...
| post 8..11 / 12..13, 14..17 / 18..19, 20..23 / 24..25 | u32 action, u16 stock x3 | the three HUD icons (2.3) with their starting stock; the stock is difficulty-adjusted at creation (2.11) | 0048fa30, 0055bc90, 0049d5e0 |
| post 26..41 | u32 x4 | four context abilities (2.3) | 0049d560, 0049d520 |
...
- **AI-035** (observed, 00569f40, 00450040, 004502e0, 004500c0). Record `ranged_kind - 1` (1 hero bow,
  ...
  range B; 43..78 grid B; 79..80 damage B. A grid is 6 distance steps x 3 skill rows of u16 percentages stored
...
| stun level | u16, clamped 0..300 | 0 | >= 70 unconscious, < 30 awake; decays per 3.11; floored at 30 while tied or carried; a player's coma sets it to 300; an "out" NPC's floor is conditional (AI-174, withheld) |
...
| per-figure usage penalties | u16 x 9 | 0 | +50 when a figure is chosen; -10 (floor 0) each time a scoring pass is entered (not on a chooser call whose attack gate fails) (3.8.2); they decide later choices, so a snapshot carries them |
| melee experience, bow experience | (whole 0..100, hundredths) x2 | the profile words | raised by kills and strikes, capped at 100 |
...
- the **civilian periodic routine's countdown** (AI-050 step 5);
- **queued combat effects** (a strike's queued messages, native 102's direct step) are owned by the sequence
  machinery's snapshot contract (`spec-script-vm.md` VM-215), not by this subsystem.
...
- **AI-045** (observed, 0055dbb0, 0050b640, 00438710, 00438600). Difficulty is 0 easy, 1 medium, 2 hard,
  ...
  A scaler applies to **hostile-side** actors only and is the identity on medium: hit points x 0.5 easy /
 x 1.5 hard (cap 10000, truncated); initiative, melee experience and ranged skill x 0.5 easy / x 2 hard (cap
  ...
  (004936f0); the punch's stun amount x 1.5 on hard (00475bd0); the arrow's target guard (3.12.3, 004859b0); the
  identification radius x 1.3 easy / x 0.7 hard (004a0a30); the reaction-delay multiplier 2 on easy and hard
...
- **AI-061** (observed, 00486fa0). Every frame: if the body facing changed, the head angle is pre-compensated
  by -(change in 16ths) x 0.3927 rad, clamped to +/-pi/2, so the head keeps pointing where it was. Target head
  angle by mode: normal / recovering 0; glancing left -pi/4, right +pi/4 (these revert to normal unless the actor
  ...
  mode (set while the actor's order state is 19). Tracking or given: effective range x 1.4 (truncated) and
...
- **AI-063** (observed, 00486fa0). Drunk wobble (drunkenness d != 0): four phases advance by 0.1, 0.07634,
  0.12321 and 0.04546 rad per frame (mod 2pi); range' = trunc(range x (1 - 0.01 d (0.1 (cos p0 + sin p1) +
  0.6))); half-angle' = half-angle x (1 - 0.01 d (0.1 (sin p2 + cos p3) + 0.2)), capped at 0.95 rad.
...
Validation (h01-measurements-2.md 6, independent evidence): the measured on-screen sector of ~80 deg becomes,
after undoing the 0.5736 projection, a world sector of 59 deg (half-angle 29.5 deg) whose axis lies on the
16-direction facing 67.5 deg: **agrees** with 0.5 rad. The measured reach (270 px along x, 196 along y, ratio
...
- **AI-066** (observed, 00489680, 00489eb0, 00489d00, 0048db80). The value is 0 when the seen element's order
  ...
  geometry. Otherwise, with v = target eye point - observer eye point and v.y x 1.7434:
  1. squared elliptical distance <= range^2, and the target in the front half-plane (axis * v >= 0) or within
     60 px (squared <= 3600);
  2. within 20 px (squared < 400): value 1.0;
  3. the **light-limited reach** L: with dz the height difference, L = sqrt(range^2 - dz^2) (0 if |dz| >= range).
     ...
     clamped to [0.5, 1]; f = 2*level - 1; L' = f x 400 + (1 - f) x L. Cached per NPC per frame;
  4. squared distance <= L'^2 and the angular test: within 60 px the target's direction in 16ths relative to the
     **body** facing must be 0..4 or 12..15 (the front 9/16 of the circle, +/-101 deg); beyond 60 px the target must
  ...
  6. the **posture factor** of the seen actor: order state 10 -> x 0.5; else by its posture code: 2, 8, 14 ->
    x 3; 3, 9 -> x 20; 4, 5, 6 -> x 2; 7, 10, 11 -> x 1.5; others x 1 (the codes' names were not mapped, 9.2).
...
- **AI-068** (observed, 00488f60, 00489b30, 00487d00). Let k = element id + frame. A **hostile** observer
  ...
  frames, value W x 2 g x 0.01 where W is the target's conspicuousness word (2.4; a target playing sprite
  ...
  the posture-scaled product stored, AI-066); for a player or civilian target the value is (2 x g) x W x 0.01 in that order,
  then **stored as a single-precision number** in the entry; each frame the contribution is trunc(stored value
 x 20) - trunc(stored value x 200) while the observer is in the wide mode - as an integer, whether the value
  was recomputed this frame or cached; "visible now" = contribution != 0; the maximum contribution over
  categories 0..2 is kept for the AI. Example: W = 80, g = 0.5 exactly: (1.0 x 80) x 0.0099999997764825821 =
...
- **AI-073** (observed, 0048a830, 005b3c40). Deafness decays per frame by 10 while < 301, else by
  50 x trunc(v / 300), then is raised to the coverage of the level's loud-sound geometry at the actor's
...
- **AI-074** (observed, 00487d00, 004a0a30, 00570e70). An NPC still shown as a silhouette that is a civilian
  ...
  any active, alive, conscious player character has it within G x f, G the base range, f = 1.95 if that
  player's order state is 16 else 1.5, x 1.3 on easy / x 0.7 on hard, the distance elliptical (y x 1.7434) and
...
- **AI-081** (observed, 00410620, 0041f540). The events are named here by what happens; the only numbers an
  ...
  *Perception*: an enemy comes into the cone [0]; a reported enemy leaves it [1]; a noise is heard [2]; a body
  ...
  a brawl is seen [32]; an arrow lands nearby [31]; a stone lands [-2]. *Actor*: reached the point [3]; could
  ...
  [24]; the adversary is weak [-2]; injured after the fight [-2]; a good strike / a lethal strike [-2]; an
  enemy is near [-2]; an arrow was launched [-2]; a door combat [50]. *Stimuli*: panic [21]; wasp / wasp gone
  [25 / 26]; apple [27]; net / net gone [28 / 29]; an apple chase comes near [49]. *Calls between NPCs*
  (delivered synchronously, the caller learns whether they were accepted): alert [33], combat alert [34],
  finish the brawl [35], hey [36], tower-guard alert [37], the tower guard calls me [38], hint [39],
  instruction [40], look there [41], coordinate [42], report [43], go to the officer [44], officer I am back
  [45], the colleague is back [46], patrol coordinate [47], you just wait [48], clean up after the brawl
  [-2]. *Dialogue beats* between two NPCs (the "my line" / "your line" steps of a reported conversation, four
  each) [-2]. **This dispatcher produces no other values**; the separate dispatcher of the sequence machinery
  produces the values 100..106 documented by `spec-script-vm.md` VM-108. Every event's behaviour is in
  section 3 under the name used here.
...
- **AI-082** (observed, 00410620). Before any state logic: (a) a script-locked AI drops every event, except
  ...
  fit-again too **while its order state is 7** - the **tied** state (AI-187; an order state, the same domain
  as the carried state 18 and the hidden states 21..23, not a sprite action id): a tied actor never wakes;
  (g) three events are converted here regardless of state:
...
with n frames; " x m" is the reaction delay of AI-100; "reset" is return to duty (AI-101). Rolls name their draws.

#### 3.5.1 Reaction delays, reset, placement

- **AI-100** (observed, 00438620 disassembled, 0055dbb0). A delay marked x m is trunc(((100 - initiative) x
  0.01) x base x m + 1) frames, evaluated in that order in extended precision with the single-precision
  ...
  multiplier is initialised to 1.0.) Examples: base 60, initiative 10: 90 x 0.0099999997764825821 =
  0.89999997988; x 60 = 53.99999879; + 1 -> trunc = **54** on medium, **108** on easy and hard (107.99999758 +
  1); initiative 0 on medium: 100 x 0.00999999977 x 60 + 1 = 60.99999866 -> **60**; initiative 100 -> 1 on
  every difficulty. The integer-boundary cases fall to the lower integer because 0.01 is stored slightly
  below 1/100.
- **AI-101** (inferred, medium; the event's handling is observed in 00424d50; the reset body itself - the
  outer AI object's first virtual, invoked with argument 0 by the return-to-duty event, the class
  initialisers' tails and the en-route handler - could not be located in the export: 9.2 item 14). **Return
  to duty** (the return-to-duty event, the unlock native, the end of a search, the finish of a brawl): the AI
  leaves its current state for the default state and the actor goes to his post, to the nearest point of his
  route or resumes the patrol (AI-096). **Observed around it** (00444bb0, 0043a220, 00435c80, 0040de30): the
  reset body does not clear the target list or the remembered position - the callers clear what they clear
  (a search clears the watched element, the colleague-found handler its search bit); an implementer must
  therefore not clear the remembered position and its priority on a reset. What the reset itself changes
  beyond the state is a hand-off boundary (`Assumption::ResetBody`).
- **AI-102b** (observed, 00435c80, 0040de30, 00414550, 00435bd0; high). **Placement** (the two class
  initialisers, hostile and friendly, run once at placement): the perception range is copied from the level
  (AI-060), every player character and enemy soldier enters category 0 (AI-070), a hostile's hit points are
  difficulty-scaled (2.11), then the initial state follows the actor's stored initial action (AI-102: 48 ->
  unconscious with stun 300; 45 / 47 -> dead; 159 -> on post and post-bound; 162 -> napping with wide
  perception and the Z marker; 270 -> on post with the hidden flag; 0 / 1 / 3 -> on post with the look-around
  interval; **any other action -> on post with a logged error**; an actor placed in a house sector -> the
  inside-house sub-state); the patrol rail is assigned (index 0, forward) and the "return to route" flag is
  raised when a rail exists; the friendly initialiser additionally arms T = 20 + rand%40 (one roll) for the
  on-post look. An on-post actor whose timer fires and who is not in a company sequence glances with the
  kind rand & 3 (one roll).
...
- **AI-105** (observed, 00433060, 00439cb0). **Sighting of an enemy (view)** in default / wondering /
  ...
  is recorded; "seen clearly" = same layer or at most 50 px below, or, if higher, flat distance^2 <= dz^2; the
...
- **AI-106** (observed, 00433830). **Noise (hear)** in the sleeping, default, wondering or seeking states, by
  ...
  pre-reaction, question mark, remark (not for kind 7), T = 1 if already in default else 60 x m; (b) already
  ...
  heard-whistle, question mark, T = 50 x m, else the heard-steps reaction. Kinds 11, 12: in default -> watching
...
- **AI-108** (observed, 00434710). **Sees an object**: object kind 3 (ale): in an ale sub-state -> queued; else
  clear the order, remark, face, question mark, wondering / ale reaction, T = 60 x m. Kinds 7 and 8 (purses):
...
- **AI-110** (observed, 00433f90). **An arrow landed**: remember with priority 5; already seeking and not an
  ...
  face, the look-there shout within 200 px, T = 60; others -> arrow reaction with T = 100 x m; the reaction's
...
- **AI-114** (observed, 00424d50, 00445d40, 004460f0, 004468a0, 0044b000, 00446400, 004469e0). **Purse /
  ...
  with the taker as rival, T = 30 x m; in a brawl already -> the taker joins the rival list; else go for another
  ...
  alerted again; a down rival is removed; **continue** iff share x 100 / (share + brawlers + 1) < the
...
- **AI-117** (observed, 00424d50, 00443d10, 00443af0). **Whistle**: heard-whistle, timer -> face the spot,
  ...
  looking for soldiers; rank 2 -> an area search around the spot with radius (whistle - 2) x 400 / 98 px; not
...
- **AI-119** (observed, 0043a220, 0043d0c0, 0043d530, 00522330, 00416800, 00438600). **Area search** (centre,
  ...
  from the mission; which chunk was not traced, 9.2) gets a cost = distance^2 to the centre + 100 per layer step;
  points within radius^2 count; with a direction, points in a 15-sector fan around it are preferred (nearest
  ahead, then to the sides); points with cost < 10^6 are inserted in cost order. If the first point's
...
- **AI-121** (observed, 0043c960, 00424d50). **Reporting to an officer** (rank 0): the nearest awake officer
 ...
 x 30 frames after which he alerts the tower guard or searches himself (radius 300); an instructed soldier
...
- **AI-123** (observed, 00424d50). **Combat alert call** (from a fight): rank-0 receivers in default /
  wondering / seeking: question mark 10, face the fight, T = 50 x m -> run to the spot (arrival 50) -> search 300
...
- **AI-126** (observed, 00424d50, 0040e7e0, 00413240). Entries: a civilian's panic (a brawl punch or a fight
  ...
  (rand%5 + 14 plus the chaser's bearing, one roll, sides alternating); runs at speed x 1.2 while the chaser is
  more than 150 px away, else x 1.0; a few more legs after the chaser gives up (at most 7), then reset. A merry
...
#### 3.5.8 Civilians' sight and hearing, the colleague found, the beggar (revision 7)

- **AI-128** (observed, 0040d440, 0040d690, 00470cc0; high). A civilian's **side** is its attitude word
  (2.4): a friendly civilian is on the player's side, a hostile one on the garrison's. *Sight* (in default or
  wondering): the seen element is fighting (has adversaries) -> panic, flee away from it; the seen element is
  on **my side** -> "admiring" (T = 60, walk toward him; the admiring remark only when the seen is the hero);
  the seen element is on the **other side**: inside a no-sight zone -> the frightened remark and hide in
  place; else target := the seen, position remembered with priority 5, "civilian enemy reaction", T = 30 ->
  then AI-127's run to the nearest idle soldier *of my side* (a merry man for a friendly civilian, a garrison
  soldier for a hostile one; none reachable -> flee). So a hostile-attitude civilian reports the hero to the
  garrison and admires soldiers; a friendly one admires the hero and reports enemy soldiers to friendly
  soldiers. *Hearing*: a running noise (kind 7) -> remember the spot; a hostile civilian runs to the nearest
  idle garrison soldier, a friendly one flees. A whistle (kind 10) is answered only by a **child**: question
  mark, walk to the whistle position, T = 70. The civilian kind (man / woman / old man / beggar / notable)
  does not enter these two routines; beggars are handled by the soldiers' seek-point questioning (AI-120).
  Fixtures: a friendly civilian sees a hostile soldier at 150 px with an idle merry man 300 px away ->
  enemy reaction T = 30, then the run to the merry man (arrival 70), then the alert call; a hostile civilian
  sees the hero -> the same with a garrison soldier; any civilian sees two duellists -> panic away from them.
- **AI-129** (observed, 00444bb0; high). **The colleague found** (the watched-element sighting while
  seeking): rank 0 with a known officer and the seen colleague a rank-0 soldier not already sent: the search
  bit "look for him" is cleared; if the colleague is already being sent or lectured -> return to duty; else
  "send the colleague to the officer": register him, the report remark, walk to him (arrival 0). Rank 1 (an
  officer) not lecturing and the colleague not sent: register him; a rank-0 colleague gets the go-to-officer
  call, the officer walks to him and "waits for the colleague", T = 10. Any other case (a knight, a civilian,
  a colleague already sent): the remark only, then the synchronisation branch: the watched element is
  cleared; if a check-for-sync is pending on this colleague and I am in a program and he is in the default
  state: his cursor index (the current one, or the previous when he is en route) equal to my stored index ->
  back "in program" and the program continues; else register as his waiter (no duplicate) -> "synchronising",
  T = 20. Otherwise walk to him (arrival 0) and: in the default state -> "colleague detected", T = 50; in
  another state -> remember my state and sub-state as the resume point, register him, the seeking "colleague
  detected", T = 50. No rolls.
- **AI-131** (observed, 0042c9a0; high). **The beggar's "betrayal" is not a roll**: the predicate is "the
  questioning soldier carries a ranged weapon" (ranged kind != 0). An archer or crossbowman questioning a
  beggar gets the pointed player as target and enters the shooting sub-state; a melee soldier asks the second
  question and moves on. Nothing depends on the player's payment or on the beggar's attitude.
...
- **AI-140** (observed, 004303e0, 004207f0, 00432d30, 00438cb0). The decision runs at the end of most
  ...
  **Required inputs**: the enemies (the other side's living, targetable actors), the friends (own side, alive,
  ...
  exists or the too-proud test (AI-145) fails: on the hostile side, when (courage x 0.045 + 1) x n <= E ->
...
- **AI-141** (observed, 00432d30). **Morale**: strength = sum over friends (a player 100, an NPC 100 + `p4`);
  r = 100 x strength / (100 x n + 1); s = r < 100 ? max(0, (r - 20) x 50 / 80) : min(100, 50 + (r - 100) x
  50 / 200); if hp < max: s = s x hp / max (integer); a rank-0 soldier with an officer among the friends: s x
...
- **AI-142** (observed, 0043d780, 0042fe00, 0043ea70). **Target choice**: the nearest valid enemy with the
  distance sqrt(dx^2 + (1.7434 dy)^2); the join from observe adds 10000 per attacker already on that enemy (prefer
...
- **AI-144** (observed, 004303e0, 00432f80, 0042ef80, 0042fe00, 00445210, 0043c960). **Fight** -> pick the
  ...
  seeing / hearing; after done / timer, if the enemy is nearer than 30 px -> immediate (T = 1), else T = 30 x m or
  10.

#### 3.7.3 Per-kind tactics

- **AI-145** (observed, 0044a090, 00424d50). **Officers** alert soldiers when a rank-0 friend exists (stand
  with the arm raised, T = 20, decide again); rank-0 soldiers near an officer get x 30 morale. **Too proud**
...
#### 3.7.4 Menacing, the tower-guard alert, the officer's rally (revision 7)

- **AI-154** (observed, 00424d50, 0042e450, 00447030, 0049f510; high). **Menacing** (top state 5, one
  sub-state). Entry: a hostile in "approaching a sleeping enemy" receives *done* with an unconscious target.
  If the target is a player character whose status carries the **coma** flag (AI-172) and nobody already
  holds him, the guard enters menacing, registers himself as the player's **holder** (one holder per player;
  the holder is the "engaged" mark the shadow test of AI-069 reads, so other soldiers stop accumulating
  shadows on that player), plays the menace remark and a stance animation unless he is a principal enemy, and
  arms T = 20. Otherwise (a non-player, or a player not in a coma): farther than 40 px (the AI's metric) ->
  walk closer (arrival 20), else "killing the sleeper" with the execution animation (AI-173). Timer loop:
  every 20 frames, while the held player is within 100 px, still unconscious and still in a coma, T = 20
  again; the moment any of those fails (revived, carried away, woken) the guard returns to duty. Sightings
  while menacing: any element other than the held player runs the ordinary sighting reaction (a second player
  approaching is attacked); seeing the held player again does nothing. Got hit: the hitter becomes the
  target. No rolls. Fixture: the hero in a coma at 30 px, a guard menacing since frame f -> he stays menacing
  through frames f + 20k; the hero revived on frame f + 45 -> the timer of frame f + 60 finds him conscious ->
  return to duty on f + 60; a second player seen on frame f + 30 -> the sighting reaction on f + 30.
- **AI-155** (observed, 0043c1a0; high). **The tower-guard alert** (the decision outcome of a tower guard,
  AI-140; the tower-guard flag is set by the mission data - its setter was not found, 9.2). The actor turns
  to face the remembered alert position, then sends the **combat-alert call** to every soldier of his side,
  alive and able, within 500 px (both the plain and the elliptical metric) that has him within perception
  range with line of sight; each accepted call counts. At least one accepted -> a turn-and-alarm sequence
  (the arm raise then the alert idle), the exclamation marker for 20 frames, the "officer giving orders"
  sub-state with T = 20 -> decide again; none accepted -> the outcome reports "nothing done" and the decision
  falls through to fight / observe. "Tower-guard observe" is the too-proud overview (AI-145). A "tower guard"
  is not a level element.
- **AI-156** (observed, 0043b1b0, 00443fa0; high; 0044dc80 - the "acceptable soldier" test - not read).
  **The officer's rally** (called by the missing-colleague alarm, an officer's body alarm, the
  tower-guard-calls-me handler and the fleeing "run to alert soldiers"; revision 6 mis-named it the
  tower-guard routine). With a position and flags: the "group" flag -> the "officer calls the group"
  sub-state, exclamation marker, T = 30, "done". Otherwise: wide perception, the search flags := "group
  search", and every **rank-0** soldier of the side that is alive, able, either not post-bound or already the
  officer's subordinate, acceptable (0044dc80), within 500 px by both metrics - at most **20** of them - is
  sent the **alert call**; each acceptor is recorded as called by this officer, his search flag set, and he
  is inserted into a distance-sorted list. Then a search direction is chosen: starting from the officer's
  facing, up to 16 sectors are tried until one has a reachable seek route; each called soldier is assigned the
  nearest seek point of the route's candidates (each point assigned once; the point, its sector and a flag
  stored on the soldier). At least one soldier called and the officer not in a house -> an instruction
  sequence (the turn, the arm animation, the group remark, the marker for 20), the "wait for the group"
  sub-state, T = 20, "done"; inside a house with a route found -> "wait inside the house", T = 50; **no
  soldier accepted -> "nothing done"**, and the caller then searches alone or (when fleeing) runs to the door.
  Fixture: an officer with three rank-0 idle soldiers at 100 / 400 / 600 px -> two calls accepted, two seek
  points assigned in cost order, "wait for the group" on the same frame with T = 20; the 600 px soldier is
  never called.
...
- **AI-151** (observed, 00485240, 00485590, 00482240, 00482fd0, 00440cf0; the exact thresholds are engine
  ...
  **Scoring** (00485590, 00482240, disassembled for the constants): the actors inside the figure's arc and
  ...
  with a class block, w = trunc(d x (100 - his defence word for this approach) x 0.01) with the
  single-precision 0.01, and s = trunc(the row's stun x (100 - his stun resistance) x 0.01); the contribution
  ...
  d = 10, defence 0, stun 0 -> w = trunc(10 x 100 x 0.0099999998) = 9 -> contribution 18, with the bonus 48; a
  ...
  **provided its score exceeds 0**; **on equal scores the earlier row wins** (a later row must strictly
  exceed); the winner's penalty rises by 50. No winning figure -> "step back" when the adversary is too far. In
  **reaction mode**
...
- **AI-153** (inferred from AI-150..152, medium). **Cadence**: there is no swing timer. The observable
  contract is the composition of the cleared rules: while in the sword fight with a target in a fighting
  state and the facing within one sixteenth, every 20 frames a step runs; the step strikes iff (a) both
  hesitation rolls fail to skip (probability (1 - (h + 1) / 100)^2 with h the hesitation value: 0.9801 for a
  hostile soldier), (b) no manoeuvre is chosen (probability 1 in a one-to-one duel or for a post-bound
  fighter, else 2 / 3), (c) the attack gate passes (probability max(skill, 50) / 100) and (d) an admissible
  figure has a scored target within its reach band and its score exceeds 0 (the jab always, when the target
  is within the row's reach). For a post-bound blue halberdier (skill 5) against one hero at 52 px: the
  per-step probability is 0.9801 x 1 x 0.5 ~ 0.49, so the expected wait between the end of one strike and
  the start of the next is about 2.04 steps ~ 41 frames; the interval between strikes is that wait plus the
  strike animation's length (from the sprite table, `ANIM`) plus any parade the hero's swings provoke
  (AI-152, T = the incoming displacement + 10). The oracle's ~5.3 s (~ 113 realised frames) between halberd
  raises is **consistent with this rule only if the animation and the parades account for about 70 frames**,
  which was not traced; the cadence stays **withheld** as an exact number (section 11). Fixture (rule-level,
  deterministic): a hostile soldier with hesitation 0 and skill 5, post-bound, one adversary at 52 px in the
  fighting state, entering the sword fight on frame f with the timer at 20; rolls fixed so that both
  hesitation rolls return >= 1 and the attack roll returns 49: the jab is issued at the step of frame f + 20;
  with the attack roll returning 50 the step issues nothing and the next step is at f + 40.
...
- **AI-164** (observed, 00475bd0, 00482fd0, 00482e50, 004801d0, 00482600, 004828b0, 00482390, 00480e90,
  ...
  facing +/- angle word 10, end = start +/- angle word 11 (half circle) or +/- 180 deg (circles), advancing by angle word
...
- **AI-166** (observed, 0047e7d0, 005c1b70, 005c1dd0 disassembled, 005c1db0, 00471ed0, 00471e50, 00472070).
  ...
  2. **Defence roll**: b = the bearing **from the victim toward the attacker** in sixteenths; for a left attack
     ...
     16: d in {0, 1, 15} -> head word 5 (front), 2..5 -> word 8 (right), 6..10 -> word 7 (behind), 11..14 -> word 6
     (left); if the attacker stands >= 20 px higher than the victim -> word 4 instead. One roll r = rand%99 + 1;
     **the blow lands iff r > word**.
  3. If it lands: **damage** = trunc(row word 3 x f), f = 1 + 0.01f x (the attacker's melee experience,
  ...
  4. **Stun roll**, whether or not the blow landed: one roll r2 = rand%99 + 1; if r2 > the victim's head word 9
     ...
     level += stun x 100 / current hit points (integer division).
  5. **Outcomes and draws.** The resolution reports two *attempt* indications, not state changes: the
     **damage indication** (the defence roll passed and the row's damage was applied to the hit points - the
     ...
     outcome: **for posture 11 always; for posture 10 only when the figure is an ordinary figure - the two
     finishing-off figures on posture 10 report "nothing"** (verified again for revision 7: the finishing-off
     figures fall through to the "nothing" result on posture 10) - the outcome value is not an effect. Finishing
     ...
     strike), draws **one more roll** rand%100 and compares it with the attacker's experience x 0.2 - the
     experience-gain message applies to player attackers only, but that roll is consumed for every attacker
     in this case. **Decision draws per strike**: a standing victim with a class block: 2 (defence, stun) + 1
     if either indication is set and the figure is a row figure; a quick strike on such a victim: 2; a victim
     without a class block: 1 for a row figure (the experience roll only), 0 for a quick strike; a lying
     victim: 0. Draws made by the reactions themselves (sounds, remarks) are accounted separately and are not
     decision draws.
...
Validation (combat-measurements.md, independent): the hero's 100 hit points, the halberdier's 80 (`p0` x 1 on
medium), the 5-hp hits (halberdier jab damage 5 x 1.05 -> 5), the click attacks never hurting him (no row), the
...
over 1.5 s (an aggregate; hypothesis: his finishing blow 20 x 1.05 = 21 plus a jab, or two hits); "two of three
...
- **AI-170** (observed, 00475bd0, 00471b00, 004a1cf0, 0048f9d0). **Energy** = 100 - fatigue (the bar's 20 px
  ...
  cap is enforced at the addition). **Recovery**: on the frames where frame mod 64 = id mod 32, when the actor
  **did not move during this tick** (his position after the movement step equals the position saved before
  it - an actor that makes no progress, e.g. blocked, satisfies this even though he has not arrived) and has
  no adversary: fatigue -= min(fatigue, rate / 10) with rate
...
- **AI-173** (observed, 00475bd0, 00472070, 00471e50, 00471ed0, 00471c00, 00463da0). **The punch** (the
  fist order -> action 123): a player without the hard-punch ability sends stun 80 (with it 150; x 1.5 on
  hard); an NPC's punch sends 40; the AI's finishing of a sleeper sends 3. The victim applies it **without any
  roll and without his stun resistance**: level += amount x 100 / current hit points (a blue halberdier at 80:
...
- **AI-174** (observed, 00471c00, 00471b00, 0048e420). **Stun level**: clamped to 0..300; > 69 -> unconscious:
  ...
  the first decrement comes after (residual + 1) frames. The console knock-out sets the level to 100.
  **Restrictions on lowering the level** (00471c00, 0049b9c0): (a) **a player in a coma** (a story hero
  felled with a clover, AI-172) has his level **set to 300** by the coma path - not a floor; (b) a **tied**
  actor and a **carried** one (order state 18) never fall below 30 (the setter floors any lower value at 30);
  (c) an **NPC marked "out"** is floored at 30 too, but only when its previous level was above 29 and the
  setter's "allow" flag is clear; **the flag is passed only by** the script's posture native (native 92: stun
  0 when it stands an actor up, 300 when it lays him unconscious), the script's property setter (native 117:
  stun := the value) and the player's revive routine (004a05a0: stun 0) - every AI and combat setter leaves it
  clear, so for them the floor applies whenever the "out" NPC's previous level was above 29 (revision 7,
  observed; the restricted-recovery contract is no longer withheld). A comrade's revive subtracts an amount
  chosen by the AI (not read).
...
- **AI-176** (observed, 0047b760, 0047b8a0 disassembled, 004df070, 004500a0). A player shooter with no
  ...
  height - target height; **stored singles** (AI-008 c): dx, sy = 1.33 x dy (the y displacement scaled by
  the single-precision 1.33 - not the 1.7434 of the AI's metric), dz, and t = tan(0.30000001192092896) =
  0.30933626; D^2 = dx^2 + sy^2 from the stored singles (an extended sum); R = the ranged record's aiming reach
  (range A, or range B when the record's flag is set); *long* = the long mode (a **non-NPC** shooter when the
  mission's outfit flag is set; NPC shooters never). If dz <= 0 (the shooter not above the target): reach = R,
  doubled if long. If dz > 0 (the shooter above): reach = R + t x dz, **doubled if long - the extension
  included**; the reach is **kept extended**, and **its square is stored as a single**; valid iff **D^2 <
  stored reach^2** (strict) in both branches. Shooting downhill extends the reach; no uphill reduction exists.
  A valid shot is
...
- **AI-177** (observed, 0047bb60, 004500c0, 004b4260). The arrow element is created bound to the shooter's
  ...
  0); p = trunc(100 x distance / range) (an integer percentage); the band is p < 20 -> steps 0..1, < 40 -> 1..2,
  ...
  **rand%100 + 1 > chance -> miss**: three further rolls jitter the aim point by -2..+2 in x, y and z, and the
  horizontal speed is scaled by 1 - (a record word x a constant). One arrow is removed from the shooter.
...
#### 3.12.4 The training target (revision 7)

- **AI-192** (observed, 004bb3a0; medium). The straw target is an **activate-by-approach object**: a click
  on it orders a walk to 6 px (16 px crouched), a turn and an activation step; it is not a bow target through
  the click. Whether its own hit test accepts the bow's intent (the shootability of objects) was not read (its
  virtual table was not recoverable from the export), so the 186 px refusal of h01-measurements-2.md 4 stays
  unexplained (9.2 item 6).
...
- **AI-185** (observed, 0058a3d0, 0058bbe0, 0046bcb0, 00412240; medium). A new movement order first
  **stops** what the actor is doing (its running actions are discarded) and then queues its own steps: a
  ...
  follow-up action. The stop reasons seen: script / AI, a cancelled special mode, AI locked. The dispatch,
  the cancellation, the modifier and the stroke rule follow (revision 7).
- **AI-186** (observed, 004de290, 005460c0, 004ccad0, 0048af40, 0048be40, 00483500, 004e0550; high).
  **The left click by icon mode and intent.** The HUD's active icon mode is a value of the character-action
  enumeration (2.3): 0 none, 1 bow, 2 / 3 punch / hard punch, 4 purse, 5 stone, 6 / 7 shield, 8 strangle,
  9 lever, 10 help to climb, 11 apple, 12 ale, 13 / 14 eat / guzzle, 15 listen, 16 heal, 17 net, 18 pay the
  beggar, 19 wasp nest, 20 whistle; choosing an icon sets it, most orders and a right click clear it; an icon
  the character's profile lacks (2.4) is not shown. **Mode 0**: the level's elements are walked from the top
  of the draw order and each is asked its own click test with an *intent*, in the order **select** (a player
  character: the selection changes, no order), **context** (the actions on humans below), **attack**; the
  first element that accepts issues the order; nothing accepting -> a walk order to the ground point (a path
  search across layers), a double click -> the run. With several characters selected only the attack, the
  act-on-arrival and the group intents are admitted (no context actions for a group). **In an icon mode**
  the target is searched with the mode's own intent and the order is that mode's action on the target: bow ->
  the shot (AI-176 / 177; refused without an arrow, on a civilian, on an arrow-immune target, or while the
  archer is already aiming); punch -> the punch order (action 123, AI-173) with an approach to 12 px along the
  target's facing plus the class angle; strangle -> its order on a standing, hostile, not immune, not "out"
  soldier with the same approach; heal, apple, stone, lever, purse, wasp nest, net, help to climb, ale, eat,
  listen, pay the beggar, shield (a walk of at least 10 px then the shield order) and whistle -> one order
  kind each, executed as the character's own action sequence (**their executors were not read: hand-off**,
  9.2 item 5). **Context actions on a human** (a plain click, intent "context"), tried in this priority and
  each gated by the clicker's abilities: (1) **search** - the NPC's purse (the `BORG` field `unknown_0x23`,
  2.10, is his purse and must be non-zero), not tied; approach to 51 px along the body's facing plus a class
  angle, action 122 (282 when the searcher is crouched); (2) **execute** - the NPC unconscious and down
  (order state 3), a hostile soldier, not immune; approach 40 px, the finishing action; (3) **tie up** -
  unconscious and down, not carried, not "out", not immune, not already tied; approach 32 px at the profile's
  tie angle; (4) **revive** - unconscious and on the clicker's side; approach 40 px; (5) **carry** - dead or
  unconscious, not immune, the clicker with either carry ability; approach 39 px; (6) none -> the attack
  intent: a standing enemy -> the attack order (AI-160). A click on a body **being carried** issues a
  hand-over chain (both turn to face, put down / take over) at 30..50 px.
- **AI-187** (observed, 0048d510, 0049bb20; high for search and tie; the carry / revive / execute / strangle
  executors were not traced, 9.2 item 5). **Effects**: **search** - when action 122's animation ends the
  searcher's purse count += the victim's purse and the victim's purse := 0 (the whole amount at once); **tie
  up** - the body is snapped beside the tier facing him, its order state becomes **7 (tied)** and the tied
  flag is set; **order state 7 is the state the pre-filter tests (AI-082 f)**, so a tied actor never receives
  fit-again: the manual's "permanent"; **carried** = order state 18 (native 89 therefore answers "is carried",
  which fits its script uses); the carry, revive, execute and strangle executions and where the finishing
  kill happens are open (the finishing strike reaches the resolver as a finishing-off figure on a lying
  victim, whose outcome is "lying" / "nothing" with no damage: AI-166).
- **AI-188** (observed, 004de840, 004cd400, 004e1fc0, 004dce20; high). **The right click**: (a) if any
  selected character is in a fight -> every fighting selected character gets the **block order** (action 76,
  then the held stance 127; AI-167) - a right click never leaves a fight; (b) otherwise in mode 0 -> per
  selected character by order state: crouched idle or crouched walk -> a stand-up step; an aiming archer -> a
  put-away step; the turn / start / stop steps are ignored; anything else -> **stop** (all pending steps
  discarded, stop reason "cancel"); (c) in an icon mode -> the mode is cancelled and the pointer reset (the
  punch, hard punch, strangle, help-to-climb, heal and pay modes also stop the character); (d) with the
  special-mode flag -> stop with the "cancelled special mode" reason, a put-away step and a mode reset. A
  left click on the ground during a fight is a walk order that stops the fight's steps; the disengagement
  then follows AI-162.
- **AI-189** (observed, 005460c0, 004ccad0, 004df070, 004ddfb0, 004de1e0, 004d0400, 004cdcc0; medium for
  the key's identity). **Act on arrival** (the manual's Ctrl-click): the pointer's act-on-arrival byte is set
  by a key-press message and cleared on release (the key is bound through the key set); with it, bow mode
  queues a **move step then the shot** ("move first, shoot on arrival"), and mode 0 on an NPC queues the
  context action deferred to arrival with a marker on the target. There is **no other queueing**: a new order
  replaces (AI-185). The three input flags are left press, right press and "dragged beyond the threshold"; a
  left drag in mode 0 is the rubber-band selection (every player in the rectangle selected), a right drag its
  counterpart.
- **AI-191** (observed, 004e1a40, 004e1890, 00547600, 00547f00, 00548050, 00548120, 00548280, 004e19c0;
  high; the side convention of the sign not fixed). **The stroke recogniser**: only while a selected
  character has an adversary is the held-button pointer path classified, per selected character, relative to
  that character's **facing**: fewer than 2 points -> a **click**; 2 points with a bounding box under 10 px in
  both axes -> a click, else nothing; 3 or more points: if the polyline **crosses itself** and the four extreme
  points (the extremes of x + y and of y - x) are in a monotone rotational order -> the **figure-eight = the
  finishing blow**; else, in order: **full circle** - the chord's angle to the facing within +/-90 deg and the four
  extremes in one cyclic order -> circle left or right by the order's sense; **half circle** - the largest
  deviation from the chord on one side divided by the chord length exceeds 0.3 while the other side's does
  not -> half circle left or right by the side; **straight** - the chord within +/-45 deg of the facing -> the
  **slow blow** ("forward"), beyond +/-135 deg (backwards) -> the **jab**; **lateral** - the chord between 45 deg and
  135 deg to a side -> attack left or right; otherwise the two-point rule. The jab targets the element under the
  pointer with the attack intent; **every drawn figure targets the character's first adversary**, not the
  pointer (the "lock" of combat-measurements.md 1.4). Which of the two lateral / circular orders is "left" is
  the sign of the cross product with the facing and was not fixed (9.2 item 16). Fixtures: a stroke of 80 px
  to the right and 20 px up while the character faces east (the oracle's) -> within 45 deg of the facing ->
  the slow blow (the 50-damage town-outfit row, AI-032); the same stroke while facing north -> lateral -> an
  attack to the side; a closed loop without a crossing -> a circle; a loop crossing once with monotone
  extremes -> the finishing blow.
...
| AI-003 | two per-NPC timers; AI 0 -> 1, rail wait keeps 0; expiry at deadline or > 10^6 frames ahead; frozen with the emoticon expiry while locked / out / paused | observed | 00416710, 0048b450, 0048b4e0, 0048a980 | high | |
...
| AI-061 | head turning, mode angles, 0.5 rad half-angle, focused x1.4 / 0.35 | observed | 00486fa0 | high | one x1.4 condition unidentified |
...
| AI-068 | per-category values, periods, x20 / x200 truncated contribution, cached values contribute | observed | 00488f60, 00489b30, 00487d00 | high | |
...
| AI-074 | identification at 1.5 / 1.95 x range with difficulty and line of sight | observed | 00487d00, 004a0a30, 00570e70 | high | oracle 120 px unexplained |
...
| AI-081 | the behavioural events and their script-visible values (-2 for the unmapped ones); no other value from this dispatcher | observed | 00410620, 0041f540, 0040dcb0 | high | |
...
| AI-149 | door fights, ladders, sleeping enemies; menacing entered | observed | 0044b940, 0044bc70, 00424d50 | medium | menacing itself: AI-154 |
| AI-154 | menacing: entry (a coma player, one holder), the 20-frame loop and its exits, sightings, got hit | observed | 00424d50, 0042e450, 00447030, 0049f510 | high | |
| AI-155 | the tower-guard alert: the combat-alert call within 500 px with sight, the alarm sequence or "nothing done" | observed | 0043c1a0 | high | the flag's setter not found |
| AI-156 | the officer's rally: up to 20 rank-0 soldiers within 500 px called, seek points assigned per sector, wait for the group / inside the house / nothing done | observed | 0043b1b0, 00443fa0 | high | 0044dc80 not read |
| AI-128 | civilians' sight and hearing by side; the child's whistle | observed | 0040d440, 0040d690, 00470cc0 | high | |
| AI-129 | the colleague found: by rank, the send-to-officer, the synchronisation branch | observed | 00444bb0 | high | |
| AI-131 | the beggar's betrayal is the questioner's ranged weapon, not a roll | observed | 0042c9a0 | high | |
| AI-102b | placement: the initialisers' effects and the initial-action rule | observed | 00435c80, 0040de30, 00414550, 00435bd0 | high | |
...
| AI-173 | the punch's stun amounts (80 / 150 / 40 / 3; x 1.5 on hard, truncated), applied without a roll | observed | 00475bd0 (disassembled at the difficulty branch), 00472070, 00471e50, 00471ed0, 00463da0 | high | |
...
| AI-185 | a new movement order stops the running actions and queues its steps | observed | 0058a3d0, 0058bbe0, 0046bcb0, 00412240 | medium | |
| AI-186 | the left click by icon mode and intent; the context actions' priority and approaches | observed | 004de290, 005460c0, 004ccad0, 0048af40, 0048be40, 00483500, 004e0550 | high | the icon modes' executors not read |
| AI-187 | search takes the whole purse; tie = order state 7 (permanent); carried = order state 18 | observed | 0048d510, 0049bb20 | high (search, tie) | carry / revive / execute / strangle effects not traced |
| AI-188 | the right click: block in a fight, stand-up / put-away / stop otherwise, mode cancel | observed | 004de840, 004cd400, 004e1fc0, 004dce20 | high | |
| AI-189 | act on arrival: move then shoot / the deferred context action; no other queueing; drags select | observed | 005460c0, 004ccad0, 004df070, 004ddfb0, 004de1e0, 004d0400, 004cdcc0 | medium | the key's identity |
| AI-191 | the stroke recogniser: crossing = finishing blow, circle, half circle (0.3), straight (45 / 135 deg), lateral; figures target the first adversary | observed | 004e1a40, 004e1890, 00547600, 00547f00, 00548050, 00548120, 00548280 | high | the left / right sign |
| AI-192 | the training target is an activate-by-approach object | observed | 004bb3a0 | medium | its bow hit test not read |
...
| POSTURE_FACTORS | x3 (codes 2, 8, 14), x20 (3, 9), x2 (4..6), x1.5 (7, 10, 11), x0.5 (order state 10) | - | 00489680 | high |
...
| PERCEPTION_VALUE | (2g) x W x 0.01 (single-precision 0.01), stored as single | - | 00488f60 | high |
...
| DEAFNESS_DECAY | 10 per frame (< 301), else 50 x trunc(v/300) | px | 0048a830 | high |
| IDENTIFY_FACTOR | 1.5 (1.95 in order state 16) x 1.3 easy / 0.7 hard | x base range | 004a0a30 | high |
...
| REACTION_DELAY | trunc((100 - p1) x 0.01f x base x m + 1); m = 1 medium, 2 easy / hard (hostile); friendly flat 3 with the outfit flag | frames | 00438620 | high |
...
| NOISE_REACTIONS | 50 quiet; 60 x m steps; 60 whistle watch; 50 x m heard-whistle; 70 + rand%60 kinds 11 / 12 | frames | 00433830 | high |
| BODY / OBJECT / ARROW REACTIONS | 80 x m; 60 x m ale; 30 purse (officer 60); 100 x m arrow (officer 60) | frames | 00434320, 00434710, 00433f90 | high |
...
| OFFICER_PATIENCE | 101 polls x 30 | frames | 00424d50 | high |
...
| OBSERVE_RULE | (courage x 0.045 + 1) x n <= E | - | 004303e0 | high |
...
| DAMAGE_FACTOR | 1 + 0.01f x experience (rank-0 soldiers only) | - | 005c1dd0 | high |
| STUN_GAIN | stun x 100 / current hp (hp != 0) | - | 00471ed0 | high |
...
| PUNCH_STUN | 80 / 150 hard punch / 40 NPC / 3 execution; x 1.5 on hard, truncated | - | 00475bd0 | high |
...
effect and returns 0** unless the row says otherwise; none blocks or yields. **Random draws**: no native
draws directly, but 140 draws transitively (its re-issued rail leg may generate the cached program roll,
AI-091) and 135 may (the return-to-duty handling it posts rolls when handled); 102 queues a direct
damage / stun step that resolves without rolls. "NPC" means a non-player human (soldier or civilian); a
player character is rejected where the row says so. Native 176 performs **no handle validation** (only the
soldier-type test): it is outside the common contract.
...
| 89 | (human) -> bool | accepts any human (player characters included - a broader type test than 134 / 135); 1 iff the actor's **order state** is 18, the **carried** state (AI-187; the tied state is 7); a non-human or invalid handle -> error, 0 | observed, 00579b10, 0049bb20 |
...
| 176 | (soldier, n) | **soldiers only** (a civilian, a player or an object -> error, no effect; a null handle is **not** checked); stores n as a 16-bit company number (2.10) | observed, 005790b0 |
...
1. **Accumulation and the shadow** (AI-068, AI-069, AI-008). One hostile soldier with element id 0 created
   ...
   and every frame (evaluated or cached) contributes trunc(0.79999995 x 20) = 15. After frame k the accumulator
...
9. **RNG decision draws of one strike** (AI-166, AI-007). A row-figure strike on a standing victim with a
   ...
   reactions' own draws (sounds, remarks) are accounted separately. **Lying-victim outcome fixtures**: a
   victim in posture 11 struck with the slow blow (row 1) -> outcome "lying", zero draws, hit points and stun
   unchanged, no reaction installed; the same victim struck with a finishing-off figure -> "lying" as well; a
   victim in posture 10 struck with the slow blow -> "lying"; **posture 10 struck with a finishing-off figure
   -> "nothing"**; in all four cases the strike handler exits without the experience roll and without an
   ordinary hit reaction.
...
11. **Stun boundaries** (AI-173, AI-174). Hit points 80, stun counter 0, level 0: a player punch (80) -> level
    ...
    comes 65 frames later and one every 65 frames after it, so the level reaches 29 after 71 x 65 = **4615
    frames** -> fit-again; with a residual counter of 10 at the moment of the punch, the first decrement comes
    after 11 frames and the level reaches 29 after 11 + 70 x 65 = 4561 frames; `t` = 0 -> never; a tied actor
    decays to 30 and stops; level 69 -> still conscious, 70 -> unconscious; a strike with stun 50 on a victim at
    0 hit points adds nothing; a player punch on hard -> trunc(80 x 1.5) = 120 -> level 150.
...
13. **Aim validity** (AI-176). Range A 250, no flag, same height: a target 249 px due east -> D^2 = 62001 <
    62500 -> valid; 250 px due east -> not valid (strict); a target 200 px due north -> D^2 = (1.33 x 200)^2 =
    70756 -> not valid, while 187 px due north (D^2 = 61857) is valid. The shooter 100 px above the target ->
    reach = 250 + 0.30933626 x 100 = 280.93; 280 px due east valid, 281 not; in the long mode (a non-NPC
    ...
    **Rounding-boundary fixtures** (AI-008 c): (i) reach 204, same height, dx 0, dy 153.38345336914062: the
    scaled y 1.33 x dy = 203.99999956 in extended precision is **stored as the single 204.0**, so D^2 = 41616 =
    reach^2 and the strict comparison rejects the shot; an implementation that keeps the extended value would
    accept it. (ii) Downhill: R = 204, dz = 1, dx = 204, dy = 8.450092315673828, not long: reach = 204 +
    0.30933626 x 1 = 204.30933626 kept extended; its square **stored as the single 41742.3046875**; D^2 ~
    41742.30666 > 41742.3046875 -> **rejected**; an implementation that stored the reach itself as a single
    before squaring would get 41742.30859375 and accept - the "square stored, reach not" boundary is
    mandatory.
14. **Oracle procedures** (tolerances): (a) the cone reach: capture the Alt cone of one soldier with a tint
    threshold that keeps pixels >= 5 % green and expect 310 x 229 px (+/- 10 %); (b) the run heard at 330 px: read
    ...
    one pixel per 320 frames; **stationary-but-not-arrived**: a hero with a walk order whose path is blocked
    so that his position does not change during the tick, no adversary -> he recovers on the same frames as a
    standing one; a *stationary duel* recording is exploratory only - passive recovery requires no
    ...
    sight.
16. **Snapshot / restore continuation** (2.7..2.9; only the state already cleared). Each case: run the
    scenario to frame s, snapshot, continue to frame e and record the observable result; then restore the
    snapshot at s and continue to e; the two continuations must be identical, and the canonical hash must
    agree at e. (a) Perception mid-accumulation: test 1's scenario, s = 33 (acc = 510, the shadow flag set,
    the entry's cached value 0.79999995); expected on both runs: the sighting event on frame 66, acc 0
    afterwards. (b) A rail wait: an actor executing a wait of 100 frames armed on frame 10, s = 50; expected:
    the program resumes on frame 110 on both runs. (c) Stun decay mid-counter: test 11's scenario with t = 64,
    s at a frame where the counter reads 20; expected: the next decrement 21 frames after s and the wake at
    the same absolute frame on both runs. (d) Energy recovery phase: id 5, fatigue 12, s = frame 3 (before
    the first recovery frame); expected: recovery on frames 5 and 69 on both runs. (e) The seek search:
    a soldier with a search list of three accepted points, s while walking to the second; expected: the same
    third point (its freshness and the accepted set are snapshot state) and the same "nothing found" frame.
    (f) Several sync waiters: two actors registered with the same partner in the order A then B, s before the
    partner reaches the waypoint; expected: A released before B on both runs. (g) An AI timer frozen by a
    lock: test 4's scenario, s = frame 30 (inside the lock interval); expected: the fire on frame 80. (h) The
    cached program roll: an actor whose roll 37 was drawn on frame 20 and not yet consumed, s = 25; expected:
    the block the roll selects at the next point is the same on both runs and no new roll is drawn.
17. **Callbacks** (AI-190). An actor flagged for scripting receives `FilterAIEvent` for a view event with an
...
2. **The random stream - an OpenSherwood decision.** The code leaves the choice open: the original uses one
   global stream shared with sounds and effects, and its draws occur in the order the level tick executes
   the rules - the script phase, then the elements in element-list order, with queued effects drawn when the
   sequence machinery processes them and synchronous calls nested in their caller (AI-007; the full account
   is (i)..(vi) below). **Recommended policy** (to be ratified by the maintainer): *equivalence target*
   **distribution-level**, not draw-for-draw - an original trace cannot be reproduced draw for draw anyway
   because the shared stream's sound and effect draws are not part of this engine. *Stream ownership*: one
   named stream for this subsystem ("AI and combat"), drawn by every rule of section 3 wherever it executes
   - including **transitively through the natives** (native 140 re-issues a rail leg, whose pass-through
   pre-check may generate the cached program roll; the unlock native may post return-to-duty whose handling
   rolls) - and by nothing else; native 102's queued direct step performs none of AI-166's defence, stun or
   experience decisions and draws nothing. *Draw ordering* (complete for the entry points that exist): (i) the
   level tick's phases are those of `spec-script-vm.md` (VM-103): the script scheduler runs its natives at its
   phase, and a native's draws occur inside that native's execution; (ii) the level tick updates **elements
...
- Revision 7 disputes **no finding of review 42**. Its two findings are answered: (1) the posture-10 outcome
  reversed in AI-166 and here, with the four lying-victim fixtures in test 9; (2) section 8 opens with the
  script-phase / element-pass / queue-processing account and native 102 is no longer a strike-roll example.
  Beyond them, revision 7 read the excluded branches: AI-128 / 129 / 131 (civilians, colleague found, the
  beggar rule), AI-154 / 155 / 156 (menacing, the tower-guard alert, the officer's rally), AI-102b
  (placement), AI-174 c's callers, AI-186..192 (the order pipeline and the training target), the cadence
  contract (AI-153), the admissibility contract (9.3), the restore fixtures (test 16), the RNG decision
  (section 8 item 2) and the clearance table (section 11).
- Revision 6 disputed **no finding of review 40**. Its 8 findings are answered in the text: (1) the
  posture-scaled product stored as a single before it is returned, with the 0.3333... x 1.5 fixture (AI-008,
  ...
  by posture (11 always; 10 for ordinary figures, "nothing" for the finishing-off figures - revision 6 had
  the posture-10 condition reversed, corrected by review 42), no ordinary reaction (AI-166 steps 1 and 5);
...
- Revision 5 disputed **no finding of review 33**. Its 10 findings are answered in the text: (1) the fade
  ...
  the 18 / 48 fixture; (3) the lying victim: nothing performed, no reaction, finishing not this resolver's
  effect; (4) the callback values corrected (-2 for arrow launched, stone, adversary weak, after-combat
  injury; the calls 35..47 as the dispatcher maps them), VM-108's 100..106 scoped out, the registry row
  replaced; (5) natives 177 (no perception re-run), 218 (a chief with a chief), 220 (reset only on duty), 176
  outside the common contract; (6) queued strike effects bound to VM-215, element-order interleaving, native
  102 without rolls, the 6.1 sentence fixed; (7) the strike target list and sweep progress, the waiters'
  registration order, the civilian countdown, queued effects owned by VM-215, the two restore cases; (8)
  recovery keyed to no movement during the tick, the blocked-actor fixture; (9) the coma's 300, the tied /
  carried floor, the "out" NPC's conditional floor withheld; (10) the six task identifiers recorded.
- Revision 4 disputed **no finding of review 27**. Its 13 findings are answered in the text: (1) the rounding
  boundaries of AI-008, AI-066 reconciled, the aim equality fixture, general equivalence not claimed; (2) test
  1's id-1 variant (first evaluation frame 1, shadow 8, sighting 67); (3) 3x penalty, the circle bonus without
...
1. (closed in revision 7: AI-154) The tower-guard flag's setter in the mission data, and the "acceptable
   soldier" test of the rally (0044dc80).
2. (closed in revision 7: AI-155, AI-156, AI-129) -
3. (closed in revision 7: AI-131, AI-128) -
4. (search and tie closed in revision 7: AI-187) The carry, revive, execute and strangle executions and the
   reviver's stun subtraction (the executor's action table at 00475bd0 could not be decoded reliably;
   0049fa20): **observable contract for the implementer** - carry: the body enters order state 18 and moves
   with the carrier until put down; revive: the body's stun falls below the waking level and AI-125's
   awakening follows; execute: a hostile unconscious soldier dies; strangle: a standing soldier is taken from
   behind (the outcome not read). Each is an `Assumption` until read.
5. (the dispatch, cancellation, modifier and stroke rule closed in revision 7: AI-186, 188, 189, 191) The
   per-kind executors of the icon-mode orders (heal, apple, stone, lever, purse, wasp nest, net, help to
   climb, ale, eat, listen, pay the beggar, shield, whistle) behind the sequence machinery; the hover
   pointer-shape catalogue (data, 004dd410 / 004cc100).
6. (the target's activation closed in revision 7: AI-192) Whether the straw target's own hit test accepts the
   bow's intent, and why the 186 px walkway shot was refused.
...
9. The posture codes 2..14 (setter 0046bd10; callers 00475bd0, 0047ac90), sprite actions 242 / 246 / 247 versus
   the documented hidden-in-leaves ids 136 / 137, the second x1.4 range condition of AI-061, and the order
...
14. The reset body (AI-101): the outer AI object's first virtual, reached only through the class descriptor
    (no address in the export); what it changes beyond the state - the callers' behaviour shows it clears
    neither the target list nor the remembered position (AI-101). The stun counter's phase when the level is
    set by a script rather than a blow.
15. (closed in revision 7: order state 18 is carried, 7 is tied - AI-187) -
...
20. Whether an element created during the level tick's element pass (a released arrow, a thrown purse) is
    updated in the same pass or first in the next: the per-element update call of 004c6ef0 was not isolated
    (its loops at the lost-check and the sequence pass are not it); still open.
21. (closed in revision 7: AI-174 c names the flag's callers) -
22. The player's kill-on-the-ground ability and the AI's execution of a sleeper as effects (00463da0 and the
    execute action's executor): what they do to the victim beyond AI-173's stun-3 message - the observable
    contract is "the victim dies" (`Assumption::ExecuteEffect`).
16. (closed in revision 7: AI-186..191) The left / right sign convention of the lateral and circular figure
    orders in the stroke recogniser (00548120, 00548050, the vector helper 005fca90) and the identity of the
    act-on-arrival key in the key set.
17. What an area search registers as "known" after clearing the watched element (AI-070), and the second
    x1.4 range condition (AI-061).

### 9.3 Material excluded from this file (expression filter), and the observable contract in its place

**Combat admissibility (AI-151).** The per-figure experience minima and hesitation maxima are a tuned-value
table the program carries as content: excluded, one line. The observable contract the implementer needs: an
NPC's repertoire grows with its melee experience in the order jab -> laterals -> slow blow -> half circles ->
circles -> finishing blow, the circles being available to a non-zero-hesitation AI regardless of experience;
what a player sees is that low-tier soldiers only jab and cut sideways, mid-tier ones add the slow blow, and
only the best add the sweeps and the finishing blow. The implementer records `Assumption::FigureAdmissibility`
with a monotone threshold set of its own and the oracle procedure of test 14(c) measures the repertoire of
each tier as outcomes; no threshold value from the executable is used.
...
## 11. Clearance table and what is still withheld

Per area, after review 42 and this revision (the reviewer's clearance is the authority; this table is the
analyst's account of what the file now offers):

| Area | Offered as individual contracts | Withheld, and why |
|---|---|---|
| Perception | lifetimes, caching and staggering, the rounding boundaries and fixtures, the noise and hearing rules | the unidentified x1.4 gate and the posture-code names (9.2 item 9: not read); the graded value's other float paths (item 18: no fixture); the oracle discrepancies (item 10: no recording) |
| State machine | the reviewed transitions, delays, ordering, callbacks and native guards; **this revision**: menacing (AI-154), the tower-guard alert (AI-155), the officer's rally (AI-156), the colleague found (AI-129), the beggar rule (AI-131), the civilian branches (AI-128), placement (AI-102b) | the reset body (item 14: not locatable in the export; the observable boundary is stated in AI-101), the tower-guard flag's setter and the rally's acceptance test (item 1) |
| Rails | cursor, opcodes, cached rolls, waiter order, native 140 styles 0 / 1 | the invalid-style deviation (a maintainer ratification, section 8 item 7) |
| Orders | AI-185's movement order; **this revision**: the click dispatch by icon mode and intent, the context actions' priority and approaches, search and tie effects, the right click, act on arrival, the stroke recogniser (AI-186..191) | the icon modes' executors and the carry / revive / execute / strangle effects (9.2 items 4 / 5: not read); the lateral sign and the key identity (item 16) |
| Combat rolls | contributions, friendly scoring, indications, decision counts, queued delivery, sweep ownership, the lying outcomes | the admissibility thresholds (9.3: content); the exact cadence (AI-153: needs the animation lengths and a trace) |
| Energy | costs, recovery amount, cadence and guard | the oracle's in-fight recovery (item 10) |
| Knock-out | punch scaling, thresholds, decay phase, coma 300, the conditional NPC floor with its callers (AI-174 c, this revision); search and tie (AI-187) | carry / revive / execute / strangle effects (item 4: not read) |
| Bow | aim boundaries and fixtures, penetration, immunity; the target as an activate-by-approach object (AI-192) | the interpolation's boundary rounding and the per-player record selection (item 11), the target's bow hit test (item 6) |
| Natives | the contracts of 6.1 | 85, 87, 88, 90, 99, 128, 130, 228, 235, 240: bodies not read |
| Snapshot / replay | the inventory of 2.7..2.9 and the restore fixtures of test 16 | integrated snapshot / replay / hash clearance: needs the implementer's fixtures for the pending event queue, the sweep and the collapsing cone (item 19) and the RNG policy's ratification |
| RNG policy | the recommended policy of section 8 item 2 | ratification (a maintainer decision) |
| Mission readiness | - | integration with navigation and the scripts and a canonical-input completion trace, outside this file |
