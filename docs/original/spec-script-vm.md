...
Status: `draft`, revision 7 (answers Codex reviews 14, 17, 21, 25, 28 and 31; awaiting re-review). Partial
clearance in force (reviews 21, 25, 28, 31): the instruction, calling and native-result protocols; the message
contracts; the scheduler-independent, individually settled native effects; the default zero for an unwritten
result slot (VM-071); VM-088 with both buffers and case 17; native 223's classification; the ordinary termination
rules of 8.1; the distinction between ordinary arrival and approach completion; the recorder guard inventory and
compound-overflow coverage (VM-201); native 47's two destinations; the 18 / 235 accounting; native 51's conditional
hold; the per-instance snapshot contents, the fault-log / suppression preservation and cases 20 / 21. Not cleared:
the interpreter core's sentinel departure and the complete snapshot / fault contract as a whole, the full
recording / scheduler integration, the camera conversion and interference, deferred execution, and every
sibling-gated effect (1.1, 4.2, 4.3). 235 is not a blanket implementation clearance. Build: GOG English edition,
...
- Analyst: 2026-09-13 and 2026-09-18, session `a45d5359e8dec5140` (Claude agent, analyst role under ADR-0009),
  revisions 1..7 (revisions 5 to 7 on 2026-09-18).
- Reviews (reviewer Codex gpt-6-astra, spec-reviewer role with access to `re/`; each review is committed under
  ...
  - review 31, `2026-09-18-codex-review-31-spec-script-vm.md`: **revision 6**, blob
    ...
    `c2c578a7…`, which carries the same blob), 4 findings, fix-then-clear with the partial clearance stated above.
    This revision 7 answers all six.
...
23. **Sentinel jump in a same-instance nested callback (VM-070, VM-095).** The level's `Hourglass(1)` calls `n109(null, 6)`; the level's `ProcessMessage(6, …)` (a nested callback on the same instance, depth 2 or more) executes the sentinel jump at its own depth → only the nested invocation ends (its return register unchanged: an earlier `0x07` value 9 in the Hourglass stays 9), `n109` returns and the Hourglass continues at its next instruction; the Hourglass then ending with 0x06 lets the engine read 9. If instead `ProcessMessage` first calls `g` with 0x05 and the sentinel jump happens inside `g`, `ProcessMessage` continues after its 0x05 (case 22's rule) and only a later 0x06 / sentinel at its own depth ends it.
...
| **deferred execution fault** — a seek element of 57 / 70 / 71 whose `actor` is not an actor-family element (no check at recording time, rows 57 / 70) | unchecked access by the actor side, long after the native returned | **no native-return outcome exists**: the recording native returned 1 and its callback may have ended. The fault is observable at the moment the element is handed over — the queue drain (VM-215, step 9) of the tick in which its level is dispatched: the element is set **refused** with the next-level propagation of VM-217 (its level never completes, later elements cancelled in chain), the attached message of 70 / 71 is **not** delivered, and `Fault::DeferredTarget(id, handle)` is recorded at that tick, attributed to the launching sequence (the class that recorded it and the tick of its launch). Nothing already returned is changed and no callback is unwound. A seek whose `actor` is valid but whose `target` is not is **not settled** here: what the seek runner does with it is behind the actor gate (VM-216 / 4.3); OpenSherwood applies the same refusal as policy until that gate opens. | UB; the outcome had to be chosen |
