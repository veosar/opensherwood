...
Status: `draft`, revision 5 (answers Codex reviews 14, 17, 21 and 25; awaiting re-review). Partial clearance in
force (reviews 21 and 25): the instruction, calling and native-result protocols; the message payload, target,
synchronous-delivery and dynamic-context rules; the individually settled, self-contained native effects; the
default zero for an unwritten result slot (VM-071). The requirement to preserve both pending buffers is
established; not cleared: the complete snapshot / fault contract and its acceptance trace, the recording /
sequence and scheduler integration, the camera conversion and interference, and every sibling-gated effect. Build: GOG English edition,
...
- Reviews (reviewer Codex gpt-6-astra, spec-reviewer role with access to `re/`; each review is committed under
  ...
  - review 25, `2026-09-18-codex-review-25-spec-script-vm.md`: **revision 4**, blob
    ...
    fix-then-clear with the partial clearance stated above. This revision 5 answers all four.
...
- Stopping statement (honest scope). **Settled from the executable:** the instruction semantics; the calling
  ...
  boundary** of 237 ids (265 minus the 12 exclusions of 4.2 and the 16 ids of 4.3) — for the ids that hand work
  to an actor or to the camera this settles what is requested, while the execution and completion of the request
  belong to the sibling specifications of 1.1; the effect of 16 further ids only up to an unresolved code, flag or
  ...
  abortion rules; messages; mission variables; the end of a level. **Not settled:** the 12 exclusions; the 16
...
| VM-201 | Native 32: not recording (level counter 0) → error, 0. Else, if the sequence has an element **and the last recorded element's level equals the current level**, level += 1; returns the level. Two consecutive barriers count once. **16-bit wrap** (original): starting at level 1, the **65,535th** increment turns the counter to 0 while the recording object stays open. Three guards then matter, and they differ per native: **30** tests the recording object → refuses (error, 0); **31 / 32** test the level counter → "not recording" (error, 0); the **common append step** tests the recording object → still appends, tagged with level 0 — verified for the timer (56) and the recorded messages (43 / 44), which have no guard of their own; **45, 59, 63 and 212** test the level counter *before* building anything → error, nothing; the remaining recording natives were not individually checked for a preliminary guard and, at level 0, either refuse on such a guard or append at level 0. Whether a level-0 tail can ever be launched is not established (31 refuses); the state is reachable only by a script that loops over recorded elements and barriers. **OpenSherwood** (departure, 8.1): a deterministic fault on the attempted transition from 65,535 to 0. | observed | 00571100, 00570f00, 00571020, 0057a020, 00575350, 00578cb0, 005740b0, 00573450, 00574a70, 005743a0 | high |
...
| natives with an unresolved effect, flag or consumer (4.3, 16 ids) | observed write / unknown effect | `UnresolvedEffect(id)` — the value is passed to the consumer verbatim; fidelity is claimed only once the consumer's spec names it |
...
### 4.3 Natives whose effect is settled only up to an unresolved code, flag or consumer (16)
...
| 57 | validation; one seek element with gait and `v` | the parameter `v` | the actor's seek runner (movement element table, kind 0x15) |
...
| 47 | `actor, loc, dir, b` | bool | "Enter the game" walking from where the actor is: if the actor is already in the entered lists, its **stored entry** is the walk origin; otherwise the actor is added to both lists with the **supplied location** `loc` as its stored entry, while its **current position** is the walk origin (the two values differ: the entry is what a later 46 / 47 / 63 reads back, the origin is where the recorded walk starts). The walk (walk / run by `b`) is recorded through the compound walk recorder toward the entry geometry derived from `loc` and the facing `(dir − 8) mod 16` (`dir` = −1 → from the actor), and a final walk element is appended **without** the recording check. → 1. Outside a recording: lists modified, then an unchecked append (U). Not an actor / not a point / bad `b` → error, 0, no effect. | E (0) / U | 005779a0 |
...
19. **Camera natives (VM-219).** *Uncontested fixture:* zoom 1.0, no scroll destination, scroll speed 0, ramp index 0, no camera element, no actor lock, a map larger than the viewport at every zoom, the corner at `c`, and `p` a point whose converted destination `d` is **axis-aligned** with `c` (`d.y = c.y`, `d.x = c.x + 200`) so that truncation of the components is exact. After `n18(p)`: the corner is unchanged in the call and the zoom is 1.0 (no fallback); the next camera update moves the corner exactly 2 px toward `d`; later updates follow the ramp (ANIM-330). When the corner lands exactly on `d` in update `k`, update `k + 1` clears the destination and resets the step length to 1.0. *Conversion fixture:* a map 300 px wide at zoom 2.0 (viewport 320 px wide) and `p` 10 px from the left edge → `n18(p)` sets the zoom to 1.0 and the destination to the origin; `n20(p)` with the same map sets the corner to the origin and the zoom to 1.0. *Active-camera fixture:* a native-42 scroll element with speed 3 is running toward `d1`; `n19(p2, 7.0)` → the element stays the camera element, the destination becomes `d2`, the corner moves 3 px per update (not 7), and the element completes on the update after landing on `d2`. `n20(p)` with a destination pending and an actor lock set by 39: the corner equals `d` on the next drawn frame, the destination is still pending and the lock is still set — assert on the frame drawn immediately after the call, before the next camera update. After `n21(2.0)` the requested zoom is 2.0 and the drawn zoom changes in later updates (count unsettled, ANIM-333); `n21(1.5)` → error, request unchanged.
...
| unchecked accesses (U) — reads (3 with `i < -1`, 6, 9, 8, 98, 144, 164, 182 - 185, 217 out of range, 223 null; 0x08 beyond the parameters) | arbitrary memory | the native returns **null / 0** (223: 0), the callback **continues**, the wrapper still pops its arity, and a `Fault::UncheckedAccess(id)` is recorded (a strict mode terminates the callback instead) | UB |
...
| barrier wrap (VM-201) | level-0 appends, unclosable recording | the callback **terminates** with `Fault::BarrierOverflow` when native 32 would turn the counter from 65,535 to 0 (its 65,535th increment from level 1) | unreachable in retail data; UB-like |
...
tick after the terminated call. Every fault is deterministic and part of the replay hash.
