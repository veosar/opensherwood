...
Status: `draft`, revision 4 (answers Codex reviews 14, 17 and 21; awaiting re-review). Partial clearance granted by
review 21: the instruction, calling and native-result protocols, the message rules and the self-contained native
contracts; the snapshot / fault contract, the sequences and the scheduler are not cleared. Build: GOG English edition,
...
- Analyst: 2026-09-13, session `a45d5359e8dec5140` (Claude agent, analyst role under ADR-0009), revisions 1..4.
- Reviews (reviewer Codex gpt-6-astra, spec-reviewer role with access to `re/`; each review is committed under
  ...
  - review 21, `2026-09-13-codex-review-21-spec-script-vm.md`: **revision 3**, blob
    ...
    fix-then-clear with the partial clearance stated above. This revision 4 answers all three.
- Exposure record: the analyst session read the decompilation of the VM, its natives, the level tick / main loop,
  the camera update, the sequence machinery, the element-table registration and the AI / actor helpers listed in
  section 10. The reviewer's own statement (review 21): it inspected the VM, the native wrappers and bodies, the
  scheduler, the sequences, the camera, the actor / AI helpers and the campaign-container helpers; **that reviewer
  context must not implement those subsystems**. No implementer session has read any of it.
...
- Stopping statement (honest scope). **Settled from the executable:** the instruction semantics; the calling
  ...
  boundary** of 240 ids (265 minus the 12 exclusions of 4.2 and the 13 ids of 4.3) — for the ids that hand work
  to an actor or to the camera this settles what is requested, while the execution and completion of the request
  belong to the sibling specifications of 1.1; the effect of 13 further ids only up to an unresolved code, flag or
  ...
  abortion rules; messages; mission variables; the end of a level. **Not settled:** the 12 exclusions; the 13
  unresolved effects of 4.3; the actor-side execution of movement, animation, speech and action elements and the AI
  event meanings, which depend on the pinned sibling revisions of 1.1 — all still *drafts awaiting review* — and on
  their open items; the open questions of section 9. The snapshot / fault contract, the scheduler and the sequence
...
### 1.1 Dependencies on sibling specifications (pinned revisions; none reviewed yet)

Every dependency below is pinned to the committed revision it was checked against. All three siblings are `draft`
and awaiting re-review; a later revision of a sibling re-opens the rows that cite it. Pins:

| Sibling | Pinned revision | Commit | Blob |
|---|---|---|---|
| `docs/original/spec-navigation.md` | revision 2 | `e5a2e0c` | `a1718e313bf841807d1d457c6bae771a1eb42328` |
| `docs/original/spec-ai-combat.md` | revision 2 | `e966b05` | `e10208494c95caf92c2b356357329a61cf014c13` |
| `docs/original/spec-movement-animation-camera.md` | revision 2 | `b0cd053` | `f9a8c498eee7d4359e0159de459f688574841439` |
...
| Walk elements (natives 45, 46, 47, 57, 63, 64, 70, 71, 212): the orders pushed, arrival, failure | navigation NAV-130 (walk sequence), NAV-142 (asynchronous path search; an empty result fails the order after "100 clock units" — the unit is **not established** there), NAV-152 (arrival within radius + 5 px); movement ANIM-208 (arrival) and its element table 3.4 (kind 0x14 row) | drafts; NAV open question 3 (completion frame of a move-to-point action) open; the failure timing is an **implementation gate** until the unit is fixed |
...
| Action codes of native 59, battle decisions of 136, action states of 259 / 260 | AI section 5.1 (59 as "a play-animation step": contradiction, 9.4); no pinned claim names the decisions or the states | **open** — 4.3 |
...
| VM-088 | The native argument buffer with its fill count and the current script-parameter buffer with its length are per instance and persist across callbacks; nested callbacks (VM-095) push and pop the native buffer symmetrically, and the script-parameter buffer is captured by the inner frame and restored when it pops. The retail files leave both empty at every callback end; a program that does not must have them snapshotted (VM-014), and their residue is consumed by the next callback exactly as the original consumes it (case 17). | inferred | 006390b0, 00634c30, 00635210, 0063a250, 0063a320 | high |
...
| VM-201 | Native 32: not recording (level counter 0) → error, 0. Else, if the sequence has an element **and the last recorded element's level equals the current level**, level += 1; returns the level. Two consecutive barriers count once. **16-bit wrap** (original): after 65535 increments the counter becomes 0 while the recording object stays open. Two different guards then apply: natives 30 / 31 / 32 test the **level counter** — 31 and 32 report "not recording" (error, 0) and 30 refuses because the recording object exists (error, 0); the **append step** of the recording natives tests the **recording object**, so a timer (56), a recorded message (43 / 44) and every other recording native still append their elements, tagged with level 0. Whether such a level-0 tail can ever be launched is not established (31 refuses); the state is reachable only by a script that loops over recorded elements and barriers. **OpenSherwood** (departure, 8.1): a deterministic fault when the counter would wrap. | observed | 00571100, 00570f00, 00571020, 0057a020, 00575350, 00578cb0 | high |
...
| VM-203 | A recording native builds one or more elements, each tagged with the current level, and appends them to the open recording; compound recorders (the walks of 45 / 46 / 47 / 57 / 63 / 64 / 70 / 71 / 212, which add door passages, waits and turns to the walk as the navigation spec's NAV-130 describes) append **several**. Outside a recording the common append step reports an error and discards the element (E) — but effects a native performs *before* its append are not undone, and three natives differ: **46** places the actor and registers it in the entered lists first, then uses the checked append (E+: placement kept, walk dropped, result 0); **47** registers the actor in the entered lists first, then records its walk through a path that appends **without** checking the recording (U: an append through a null recording); **63** tests the level counter *before* anything else (E: no registration, nothing). Every other recording native has no effect before its append. | observed | 0057a020, 00577650, 005779a0, 00574a70, 00582640, 00585500 | high |
...
| VM-231 | **Actor-side completion** (per the movement spec's element table, draft): actor-immediate categories (VM-212 c) are *done* as soon as the actor executes them; **speak** (62 / 69) when the speech ends (ANIM-140, inferred there); **walks** on arrival within radius + 5 px (NAV-152 / ANIM-208) or refusal on a path failure (NAV-142); **seek** (57 / 70 / 71) on arrival, or at once when the seeker is the target; **animation 49** completes after the clip; **animation 50 (loop) never completes** — its level never finishes and every later level is blocked; **animation 51** completes after the clip, and the actor is then observed **holding the clip's last frame** until another element or order replaces it (the hold does not belong to the sequence and blocks nothing); **actions of native 59**: end of the action (**open**); corpse 63 / 65 (**open**); the admission and priority rules (**open**). | inferred (from the sibling drafts) | 00467230, 004646e0, 00464b20 | medium |
...
| natives with an unresolved effect, flag or consumer (4.3, 13 ids) | observed write / unknown effect | `UnresolvedEffect(id)` — the value is passed to the consumer verbatim; fidelity is claimed only once the consumer's spec names it |
...
### 4.3 Natives whose effect is settled only up to an unresolved code, flag or consumer (13)

Observing a write does not settle its effect; each row names what is unresolved and who would settle it.

| Id | Unresolved | Consumer that would settle it |
|---|---|---|
| 45, 212 | the variant the modes 2 / 3 mark on the walk elements | the actor's walk runner (`spec-navigation.md` NAV-130 does not name it) |
| 46 | the entry-position derivation from the point and facing; the entered lists' use after the first placement | the map's entry-placement rule (navigation) |
| 47 | as 46, plus the compound recorder's added elements | navigation NAV-130 |
| 57 | the parameter `v` of the seek | the actor's seek runner (movement element table, kind 0x15) |
| 59 | the action each code 0..20 requests and its completion | the actor's action runner (AI section 5.1 contradicts, 9.4) |
| 62 | the speak flag | the speech element (movement ANIM-140) |
| 63 | the take element's completion | the actor's take runner |
| 136 | the decision each code 0..16 selects; permanent vs temporary forcing | the AI's battle decision (`spec-ai-combat.md`) |
| 165 | two per-character values it writes besides the team membership | the campaign / character state (`spec-campaign-camp-saves.md`) |
| 254 | the per-element flag it writes | not identified |
| 259, 260 | the eighteen action states 0..17 | the actor's action state (AI / movement) |

Implementable as verbatim pass-through of the value; cleared only together with the sibling claim that names the
consumer (`UnresolvedEffect(id)`).
...
| 18 | `loc` | bool | **Scroll request** (ANIM-331): raw scroll target := point `loc`; scroll destination := the converted point (VM-219: viewport centred on it, truncated, bounded and aligned for the current zoom, with the zoom-reset fallback); **scroll step length := 2.0 map px per camera update**. Nothing else is written: the camera does not move in the call, no element is created (nothing waits for the scroll), the scroll **speed** left by the last scroll element and the **ramp position** are preserved — so the 2.0 governs only the first update and only when that speed is 0; afterwards the ramp rule of ANIM-330 applies. Returns 1. Null → error, 0. | E (0) | 00571270 |
| 19 | `loc, f` | bool | As 18 with step length := `f` (same caveat: effective on the first update, only with a zero leftover speed). | E (0) | 00571330 |
| 20 | `loc` | bool | **Jump** (ANIM-332): camera corner := point `loc` converted as in 18 (immediately, before the next drawn frame); the cached view is invalidated so the next frame is fully redrawn. Touches neither the scroll destination, the step length, the zoom nor the **actor lock**. Returns 1. Null → error, 0. | E (0) | 005713f0 |
...
| 46 | `actor, loc, dir, b` | bool | "Enter the game" — **immediate effects first**: if `actor` is not yet in the recording's entered lists it is added to both lists and placed at an **entry position derived from point `loc` and the facing** `dir` (−1 = the actor's own facing; the derivation is the map's entry-placement rule, not the point itself — navigation dependency, 4.3), facing set, movement stopped; if it is already listed, its stored entry is updated to `loc` instead. **Then** one walk element (walk if `b` = 0, run otherwise) is built and appended through the checked append. Outside a recording the placement / list effects still happen and only the walk is dropped (E+). Not an actor / not a point / bad `b` → error before any effect. **Returns 0 always** (the low byte is explicitly cleared). | E / E+ (0) | 00577650 |
| 47 | `actor, loc, dir, b` | bool | "Enter the game" from the actor's current position: the actor is registered in the entered lists (its stored entry taken from the lists when already registered, else created from its current position); then a walk (walk / run by `b`) is recorded through the compound walk recorder and a final walk element is appended **without** the recording check; the facing used is `(dir − 8) mod 16` (`dir` = −1 → from the actor). → 1. Outside a recording: lists modified, then an unchecked append (U). Not an actor / not a point / bad `b` → error, 0, no effect. | E (0) / U | 005779a0 |
...
| 59 | `actor, code, arg` | bool | Records an action element of `actor` (known element) by `code` 0..20: the native validates the code and, for 4, that `arg` is a valid table index, for 5 and 8..16 that `arg` is an element; 17 / 18 require a soldier; 1 passes `arg mod 16` as a direction. Which action each code requests, and its duration, is **not settled here** (4.3: the consumer is the actor's action runner; AI section 5.1 reads 59 differently, 9.4). Other codes / invalid `arg` → error, 0; else 1. | E (0) | 00573450 |
...
5. **Outside a recording (VM-203).** `n56(5)` alone → error, nothing scheduled; `n43(null, 1)` alone → no `ProcessMessage`; `n46(actor, p, -1, 0)` alone, with `actor` idle at `q` and `p` a point whose derived entry position is `p'` (fixture: a point in open ground with the entry rule of the navigation spec giving a known `p'`) → the actor's position becomes `p'`, it is in both entered lists (a following `n46` for the same actor takes the "already listed" branch), no walk is recorded, result 0; `n63(pc, corpse, 0)` alone → error, 0, the PC not registered, position unchanged; `n47(...)` alone → OpenSherwood fault (U); `n109(null, 1)` → `ProcessMessage(1,0,0)` runs before `n109` returns.
...
19. **Camera natives (VM-219).** Fixture: zoom 1.0, no scroll pending, scroll speed 0, ramp position 0, a map larger than the viewport at every zoom, `p` a point whose converted destination `d` lies inside the bounds. After `n18(p)`: the corner is unchanged in the call, the zoom is unchanged (the fallback did not fire), no camera element exists, the next camera update moves the corner 2 px toward `d` and later updates follow the ramp (ANIM-330); a scroll element that was still running keeps its speed, so `n19(p, 7.0)` issued while a native-42 scroll with speed 3 is active moves 3 px per update, not 7. When the corner lands exactly on `d` in update `k`, update `k + 1` clears the destination and resets the step length to 1.0. After `n20(p)` the corner equals `d` on the next drawn frame, a pending destination is still pending, and an actor lock set by native 39 is still set. After `n21(2.0)` the requested zoom is 2.0, the drawn zoom changes in later updates (count unsettled, ANIM-333), and `n21(1.5)` → error, request unchanged.
...
| jump to `0xFFFFFFFF` (VM-070) | unchecked fetch | the callback **ends** as by 0x06 (return register unchanged); pending 0x02 / 0x0B cells stay in their buffers as after any 0x06 | the script's intent; UB |
| unchecked accesses (U) — reads (3 with `i < -1`, 6, 9, 8, 98, 144, 164, 182 - 185, 217 out of range; 0x08 beyond the parameters) | arbitrary memory | the native returns **null / 0**, the callback **continues**, the wrapper still pops its arity, and a `Fault::UncheckedAccess(id)` is recorded (a strict mode terminates the callback instead) | UB |
| unchecked accesses (U) — writes (0 with `k < 0`, 67 / 68 / 72 / 73 / 254 out of range, 179 / 186 - 189 / 223 null, 47 outside a recording, 0x0B overflow) | arbitrary memory | the callback **terminates** with `Fault::UncheckedAccess(id)`; no partial write; the instance's buffers are cleared as at a callback end | UB |
...
| fatal routine (F: scroll callback overlap) | termination | the callback **terminates** with `Fault::Fatal`; the level is marked failed | crash |
| null dereference (C: missing callback) | crash | the engine's call is a **no-op** that leaves the return register unchanged and reads it (0 unless an earlier callback set it); recorded once per class and name as `Fault::MissingCallback` without terminating anything | crash; the retail data never lacks a called name |
...
| barrier wrap (VM-201) | level-0 appends, unclosable recording | the callback **terminates** with `Fault::BarrierOverflow` on the 65536th increment | unreachable in retail data; UB-like |
...
A terminated callback ends the engine's call as if the frame had returned with 0x06; the engine then continues its
tick. Every fault is deterministic and part of the replay hash.
...
- (Resolved at the pinned movement revision `b0cd053`: ANIM-331 / ANIM-332 now agree with rows 18 - 20 and VM-219.)
...
oracle evidence is cited there). Sibling specifications referenced, at the pinned revisions of 1.1 (all drafts awaiting review):
`docs/original/spec-navigation.md` `e5a2e0c` (NAV-130, NAV-142, NAV-152, NAV-172, NAV-190, NAV-191, section 5),
...
`e966b05` (AI-041, AI-066, AI-074, AI-081, AI-082, AI-084, AI-091, AI-093, AI-097, AI-190, sections 2.2, 3.4, 5.1);
decision `docs/decisions/ADR-0010-logic-frame.md`; reviews `docs/decisions/reviews/2026-09-13-codex-review-14-spec-script-vm.md`,
`-17-`, `-21-` (identity block). Tests that will depend on this
