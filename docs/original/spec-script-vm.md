...
Status: `draft`, revision 6 (answers Codex reviews 14, 17, 21, 25 and 28; awaiting re-review). Partial clearance
in force (reviews 21, 25, 28): the instruction, calling and native-result protocols; the message contracts; the
individually settled, self-contained native effects; the default zero for an unwritten result slot (VM-071);
VM-088 with both buffers and case 17; native 223's classification; the ordinary termination rules of 8.1; the
distinction between ordinary arrival and approach completion; the request / completion / admission ordering of
native 51. Not cleared: the full recording / scheduler integration, the camera conversion and interference, the
remaining fault / restore cases, and every sibling-gated effect (1.1, 4.2, 4.3). Build: GOG English edition,
...
- Analyst: 2026-09-13 and 2026-09-18, session `a45d5359e8dec5140` (Claude agent, analyst role under ADR-0009),
  revisions 1..6 (revisions 5 and 6 on 2026-09-18).
- Reviews (reviewer Codex gpt-6-astra, spec-reviewer role with access to `re/`; each review is committed under
  ...
  - review 28, `2026-09-18-codex-review-28-spec-script-vm.md`: **revision 5**, blob
    ...
    fix-then-clear with the partial clearance stated above. This revision 6 answers all five.
...
| 57 | `actor, target, k, v` | bool | Records "seek actor `target`" (walk if `k` = 1 else run) with the seek parameter `v` (unresolved, 4.3); **no check** of `actor` or `target` before the element is built (unchecked at execution if they are not actors: U at the actor side, 8.1); byte = recording result. | E (0) / U | 00573d10 |
...
| 70 | `actor, target, k, v, reporter, msg` | bool | Records "seek `target`" (walk if `k` = 0 else run) with the seek parameter `v` (the same unresolved parameter as 57's, 4.3) and an attached message `(msg, 0, 0)` to `reporter` (null = level) sent when the seek ends → 1; `reporter` not an actor → error, 0; `actor` / `target` unchecked as in 57. | E (0) / U | 00573da0 |
...
| jump to `0xFFFFFFFF` (VM-070) | unchecked fetch | **not a termination**: the *current frame* is popped exactly as by 0x06 — at depth 1 the callback ends (return register unchanged), at depth > 1 control returns to the caller frame's return pc; pending 0x02 / 0x0B cells stay in their buffers as after any 0x06; recorded as `Fault::SentinelJump` (once per class and address) | the script's intent; UB |
| unchecked accesses (U) — native reads (3 with `i < -1`, 6, 9, 8, 98, 144, 164, 182 - 185, 217 out of range, 223 null; 57 / 70 / 71 executed on a non-actor) | arbitrary memory | the native returns **null / 0** (223: 0), the callback **continues**, the wrapper still pops its arity, and a `Fault::UncheckedAccess(id)` is recorded (a strict mode terminates the callback instead) | UB |
...
| fatal routine (F: scroll callback overlap) | termination of the process, detected **before** the attempted callback's frame is pushed | **not a termination of any script frame**: the attempted scroll callback is **not run** (no frame exists), the current scroll stays the *enclosing* scroll, the native / element that requested it (a take-scroll element run from a native 109 / 110 inside a scroll callback) returns normally with its usual result, and the enclosing callback continues; recorded as `Fault::ScrollOverlap` | crash |
...
faults (kind, id / address, tick `T`, instance) — and the suppression sets that make "recorded once" reproducible
(the set of (class, name) pairs already recorded for `Fault::MissingCallback`, the set of (class, address) pairs
for `Fault::SentinelJump`) are authoritative state, hashed and restored with the rest. Every fault is
deterministic and part of the replay hash.
...
boundary; **the fault log and the suppression sets** of 8.1. Element flags read by natives belong to their
owners' snapshots. Restoring at a tick boundary reproduces the following ticks exactly (cases 17, 20, 21).
