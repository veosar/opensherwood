...
| P4 40 (part) | Window hygiene | **Done.** RPC EOF resumes ticking, focus loss releases held input, pointer remap on resize, bounded scale, non-sRGB surface format chosen explicitly (window pixels equal the CPU frame, measured), event loop sleeps until the next tick instead of polling. |
