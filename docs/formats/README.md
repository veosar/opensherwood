...
| Sprite animations | (layout of `.rhs` animation lists) | 16-direction action blocks, action ids, timing | action ids and their roles identified for the movement, alert, combat and state sets; timing / displacement read from the files at run time (reading rules in the spec); original timing unverified | [sprite-animations.md](sprite-animations.md) |
