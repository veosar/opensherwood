...
| (artifact directory) | `saves/*.json` (quick, auto and named saves) and `settings.json` (options) live under `--artifacts` (or `OPENSHERWOOD_ARTIFACTS`; default `harness/out`), together with `profiles.json` (player profiles) |
